...
func TestVersion01(t *testing.T) {
	...
	require.Equal(t, model.PaymentMethod, Archive.CreditTransform)
	...
	require.NotNil(t, model.DebtorAgent.PaymentSysCode, Archive.PaymentSysUSABA)
	...
	require.Equal(t, model.CreditTransTransaction.PayCategoryType, Archive.IntraCompanyPayment)
	require.Equal(t, model.CreditTransTransaction.PayRequestType, Archive.DrawDownRequestCredit)
	...
	require.Equal(t, model.CreditTransTransaction.ChargeBearer, Archive.ChargeBearerSLEV)
	...
	require.Equal(t, model.CreditTransTransaction.Document.CodeOrProprietary, Archive.CodeCINV)
...
func DrawdownRequestDataModel() MessageModel {
	...
	message.PaymentMethod = Archive.CreditTransform
	message.RequestedExecutDate = fedwire.ISODate(civil.DateOf(time.Now()))
	message.Debtor = Archive.PartyIdentify{
		Name: "Corporation A",
		Address: Archive.PostalAddress{
			StreetName:     "Avenue of the Fountains",
			BuildingNumber: "167565",
	...
	message.CreditTransTransaction = CreditTransferTransaction{
		...
		PayCategoryType:      Archive.IntraCompanyPayment,
		PayRequestType:       Archive.DrawDownRequestCredit,
		Amount: Archive.CurrencyAndAmount{
			Amount:   6000000.00,
			Currency: "USD",
		},
		ChargeBearer: Archive.ChargeBearerSLEV,

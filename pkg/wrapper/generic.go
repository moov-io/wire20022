...
// MessageWrapper provides a generic, type-safe wrapper for ISO 20022 message processing
...
// NewMessageWrapper creates a new generic message wrapper with type safety
...
// CheckRequireField validates required fields using the configured validator
func (w *MessageWrapper[M, V]) CheckRequireField(model M) error {
...
// MessageWrapperInterface defines the common interface that all wrappers should implement
// This allows existing code to work with both generic and legacy wrappers
type MessageWrapperInterface interface {
	GetHelp() (string, error)
}

// Ensure our generic wrapper implements the interface
var _ MessageWrapperInterface = (*MessageWrapper[any, string])(nil)
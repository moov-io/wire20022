...
// It uses the EndpointGapReport.DocumentWith function to create the document structure
...
func (w *EndpointGapReportWrapper) CreateDocument(modelJson []byte, version EndpointGapReport.CAMT_052_001_VERSION) ([]byte, error) {
	// Unmarshal the JSON string into the MessageModel
	var model EndpointGapReport.MessageModel
	err := json.Unmarshal(modelJson, &model)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal JSON to MessageModel: %w", err)
	}
	// Create the XML document
	doc, err := EndpointGapReport.DocumentWith(model, version)
	if err != nil {
...
// It unmarshals the JSON string into a MessageModel, creates the XML document using the EndpointGapReport.DocumentWith function,
...
func (w *EndpointGapReportWrapper) ValidateDocument(modelJson string, version EndpointGapReport.CAMT_052_001_VERSION) error {
	// Unmarshal the JSON string into the MessageModel
	var model EndpointGapReport.MessageModel
...
// It uses the EndpointGapReport.CheckRequiredFields function to perform the validation.
//
// Parameters:
...
func (w *EndpointGapReportWrapper) CheckRequireField(model EndpointGapReport.MessageModel) error {
	return EndpointGapReport.CheckRequiredFields(model)
}

// ConvertXMLToModel converts a camt.052 XML document into a MessageModel.
// It uses the EndpointGapReport.MessageWith function to parse the XML data and populate the MessageModel.
//
// Parameters:
// - xmlData: A byte slice containing the camt.052 XML document to be converted.
//
// Returns:
// - EndpointGapReport.MessageModel: The parsed MessageModel.
// - error: An error if the XML parsing fails.
func (w *EndpointGapReportWrapper) ConvertXMLToModel(xmlData []byte) (EndpointGapReport.MessageModel, error) {
	model, err := EndpointGapReport.MessageWith(xmlData)
	if err != nil {
		return EndpointGapReport.MessageModel{}, fmt.Errorf("failed to convert XML to model: %w", err)
	}
...
func (w *EndpointGapReportWrapper) GetHelp() (string, error) {
	// Build the MessageHelper structure
	helper := EndpointGapReport.BuildMessageHelper()

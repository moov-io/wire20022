...
// It uses the ActivityReport.DocumentWith function to create the document structure
...
func (w *ActivityReportWrapper) CreateDocument(modelJson []byte, version ActivityReport.CAMT_052_001_VERSION) ([]byte, error) {
	// Unmarshal the JSON string into the MessageModel
	var model ActivityReport.MessageModel
	err := json.Unmarshal(modelJson, &model)
	if err != nil {
		return nil, fmt.Errorf("failed to unmarshal JSON to MessageModel: %w", err)
	}
	// Create the XML document
	doc, err := ActivityReport.DocumentWith(model, version)
	if err != nil {
...
// It unmarshals the JSON string into a MessageModel, creates the XML document using the ActivityReport.DocumentWith function,
...
func (w *ActivityReportWrapper) ValidateDocument(modelJson string, version ActivityReport.CAMT_052_001_VERSION) error {
	// Unmarshal the JSON string into the MessageModel
	var model ActivityReport.MessageModel
...
// It uses the ActivityReport.CheckRequiredFields function to perform the validation.
//
// Parameters:
...
func (w *ActivityReportWrapper) CheckRequireField(model ActivityReport.MessageModel) error {
	return ActivityReport.CheckRequiredFields(model)
}

// ConvertXMLToModel converts a camt.052 XML document into a MessageModel.
// It uses the ActivityReport.MessageWith function to parse the XML data and populate the MessageModel.
//
// Parameters:
// - xmlData: A byte slice containing the camt.052 XML document to be converted.
//
// Returns:
// - ActivityReport.MessageModel: The parsed MessageModel.
// - error: An error if the XML parsing fails.
func (w *ActivityReportWrapper) ConvertXMLToModel(xmlData []byte) (ActivityReport.MessageModel, error) {
	model, err := ActivityReport.MessageWith(xmlData)
	if err != nil {
		return ActivityReport.MessageModel{}, fmt.Errorf("failed to convert XML to model: %w", err)
	}
...
func (w *ActivityReportWrapper) GetHelp() (string, error) {
	// Build the MessageHelper structure
	helper := ActivityReport.BuildMessageHelper()

package ReturnRequestResponse

import (
	"reflect"

	camt029 "github.com/moov-io/fedwire20022/gen/ReturnRequestResponse_camt_029_001_09"
	model "github.com/moov-io/wire20022/pkg/models"
)

type Status string

const (
	ReturnRequestAccepted   Status = "CNCL"
	ReturnRequestRejected   Status = "RJCR"
	ReturnRequestPending    Status = "PDCR"
	PartiallyExecutedReturn Status = "PECR"
)

type Reason struct {
	//Party that issues the cancellation request.
	Originator string
	//Specifies the reason for the cancellation.
	Reason string
	//Further details on the cancellation request reason.
	AdditionalInfo string
}

func Party40Choice2From(p model.Agent) (camt029.Party40Choice2, *model.ValidateError) {
	var result camt029.Party40Choice2
	var Agt camt029.BranchAndFinancialInstitutionIdentification62
	var FinInstnId camt029.FinancialInstitutionIdentification182
	var ClrSysMmbId camt029.ClearingSystemMemberIdentification22
	if p.PaymentSysCode != "" {
		err := camt029.ExternalClearingSystemIdentification1Code(p.PaymentSysCode).Validate()
		if err != nil {
			return camt029.Party40Choice2{}, &model.ValidateError{
				ParamName: "PaymentSysCode",
				Message:   err.Error(),
			}
		}
		Cd := camt029.ExternalClearingSystemIdentification1Code(p.PaymentSysCode)
		ClrSysMmbId.ClrSysId = camt029.ClearingSystemIdentification2Choice2{
			Cd: &Cd,
		}
	}
	if p.PaymentSysMemberId != "" {
		err := camt029.Max35Text(p.PaymentSysMemberId).Validate()
		if err != nil {
			return camt029.Party40Choice2{}, &model.ValidateError{
				ParamName: "PaymentSysMemberId",
				Message:   err.Error(),
			}
		}
		ClrSysMmbId.MmbId = camt029.Max35Text(p.PaymentSysMemberId)
	}
	if !isEmpty(ClrSysMmbId) {
		FinInstnId.ClrSysMmbId = &ClrSysMmbId
	}
	if p.BankName != "" {
		err := camt029.Max140Text(p.BankName).Validate()
		if err != nil {
			return camt029.Party40Choice2{}, &model.ValidateError{
				ParamName: "BankName",
				Message:   err.Error(),
			}
		}
		Nm := camt029.Max140Text(p.BankName)
		FinInstnId.Nm = &Nm
	}
	if !isEmpty(p.PostalAddress) {
		PstlAdr, err := PostalAddress241From(p.PostalAddress)
		if err != nil {
			err.InsertPath("PostalAddress")
			return camt029.Party40Choice2{}, err
		}
		FinInstnId.PstlAdr = &PstlAdr
	}
	if !isEmpty(FinInstnId) {
		Agt.FinInstnId = FinInstnId
	}
	if !isEmpty(Agt) {
		result.Agt = &Agt
	}
	return result, nil
}
func Party40Choice2To(p camt029.Party40Choice2) model.Agent {
	var result model.Agent
	if !isEmpty(p.Agt) {
		if !isEmpty(p.Agt.FinInstnId) {
			if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId) {
				if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.ClrSysId) {
					if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.ClrSysId.Cd) {
						result.PaymentSysCode = model.PaymentSystemType(*p.Agt.FinInstnId.ClrSysMmbId.ClrSysId.Cd)
					}
				}
				if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.MmbId) {
					result.PaymentSysMemberId = string(p.Agt.FinInstnId.ClrSysMmbId.MmbId)
				}
			}
			if !isEmpty(p.Agt.FinInstnId.Nm) {
				result.BankName = string(*p.Agt.FinInstnId.Nm)
			}
			if !isEmpty(p.Agt.FinInstnId.PstlAdr) {
				result.PostalAddress = PostalAddress241To(*p.Agt.FinInstnId.PstlAdr)
			}
		}
	}
	return result
}
func Party40Choice1From(p model.Agent) (camt029.Party40Choice1, *model.ValidateError) {
	var result camt029.Party40Choice1
	var Agt camt029.BranchAndFinancialInstitutionIdentification61
	var FinInstnId camt029.FinancialInstitutionIdentification181
	var ClrSysMmbId camt029.ClearingSystemMemberIdentification21
	if p.PaymentSysCode != "" {
		err := camt029.ExternalClearingSystemIdentification1CodeFixed(p.PaymentSysCode).Validate()
		if err != nil {
			return camt029.Party40Choice1{}, &model.ValidateError{
				ParamName: "PaymentSysCode",
				Message:   err.Error(),
			}
		}
		Cd := camt029.ExternalClearingSystemIdentification1CodeFixed(p.PaymentSysCode)
		ClrSysMmbId.ClrSysId = camt029.ClearingSystemIdentification2Choice1{
			Cd: &Cd,
		}
	}
	if p.PaymentSysMemberId != "" {
		err := camt029.RoutingNumberFRS1(p.PaymentSysMemberId).Validate()
		if err != nil {
			return camt029.Party40Choice1{}, &model.ValidateError{
				ParamName: "PaymentSysMemberId",
				Message:   err.Error(),
			}
		}
		ClrSysMmbId.MmbId = camt029.RoutingNumberFRS1(p.PaymentSysMemberId)
	}
	if !isEmpty(ClrSysMmbId) {
		FinInstnId.ClrSysMmbId = ClrSysMmbId
	}
	if !isEmpty(FinInstnId) {
		Agt.FinInstnId = FinInstnId
	}
	if !isEmpty(Agt) {
		result.Agt = &Agt
	}
	return result, nil
}
func Party40Choice1To(p camt029.Party40Choice1) model.Agent {
	var result model.Agent
	if !isEmpty(p.Agt) {
		if !isEmpty(p.Agt.FinInstnId) {
			if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId) {
				if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.ClrSysId) {
					if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.ClrSysId.Cd) {
						result.PaymentSysCode = model.PaymentSystemType(*p.Agt.FinInstnId.ClrSysMmbId.ClrSysId.Cd)
					}
				}
				if !isEmpty(p.Agt.FinInstnId.ClrSysMmbId.MmbId) {
					result.PaymentSysMemberId = string(p.Agt.FinInstnId.ClrSysMmbId.MmbId)
				}
			}
		}
	}
	return result
}
func PostalAddress241From(param model.PostalAddress) (camt029.PostalAddress241, *model.ValidateError) {
	var Dbtr_PstlAdr camt029.PostalAddress241

	// Flag to track if any field is set
	hasData := false

	// Check and set each field if not empty
	if param.StreetName != "" {
		err := camt029.Max70Text(param.StreetName).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "StreetName",
				Message:   err.Error(),
			}
		}
		StrtNm := camt029.Max70Text(param.StreetName)
		Dbtr_PstlAdr.StrtNm = &StrtNm
		hasData = true
	}
	if param.BuildingNumber != "" {
		err := camt029.Max16Text(param.BuildingNumber).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "BuildingNumber",
				Message:   err.Error(),
			}
		}
		BldgNb := camt029.Max16Text(param.BuildingNumber)
		Dbtr_PstlAdr.BldgNb = &BldgNb
		hasData = true
	}
	if param.BuildingName != "" {
		err := camt029.Max35Text(param.BuildingName).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "BuildingName",
				Message:   err.Error(),
			}
		}
		BldgNm := camt029.Max35Text(param.BuildingName)
		Dbtr_PstlAdr.BldgNm = &BldgNm
		hasData = true
	}
	if param.Floor != "" {
		err := camt029.Max70Text(param.Floor).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "Floor",
				Message:   err.Error(),
			}
		}
		Floor := camt029.Max70Text(param.Floor)
		Dbtr_PstlAdr.Flr = &Floor
		hasData = true
	}
	if param.RoomNumber != "" {
		err := camt029.Max70Text(param.RoomNumber).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "RoomNumber",
				Message:   err.Error(),
			}
		}
		Room := camt029.Max70Text(param.RoomNumber)
		Dbtr_PstlAdr.Room = &Room
		hasData = true
	}
	if param.PostalCode != "" {
		err := camt029.Max16Text(param.PostalCode).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "PostalCode",
				Message:   err.Error(),
			}
		}
		PstCd := camt029.Max16Text(param.PostalCode)
		Dbtr_PstlAdr.PstCd = &PstCd
		hasData = true
	}
	if param.TownName != "" {
		err := camt029.Max35Text(param.TownName).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "TownName",
				Message:   err.Error(),
			}
		}
		TwnNm := camt029.Max35Text(param.TownName)
		Dbtr_PstlAdr.TwnNm = &TwnNm
		hasData = true
	}
	if param.Subdivision != "" {
		err := camt029.Max35Text(param.Subdivision).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "Subdivision",
				Message:   err.Error(),
			}
		}
		CtrySubDvsn := camt029.Max35Text(param.Subdivision)
		Dbtr_PstlAdr.CtrySubDvsn = &CtrySubDvsn
		hasData = true
	}
	if param.Country != "" {
		err := camt029.CountryCode(param.Country).Validate()
		if err != nil {
			return camt029.PostalAddress241{}, &model.ValidateError{
				ParamName: "Country",
				Message:   err.Error(),
			}
		}
		Ctry := camt029.CountryCode(param.Country)
		Dbtr_PstlAdr.Ctry = &Ctry
		hasData = true
	}

	// If no data was set, return an empty struct
	if !hasData {
		return camt029.PostalAddress241{}, nil
	}

	return Dbtr_PstlAdr, nil
}
func PostalAddress241To(p camt029.PostalAddress241) model.PostalAddress {
	var result model.PostalAddress
	if !isEmpty(p.StrtNm) {
		result.StreetName = string(*p.StrtNm)
	}
	if !isEmpty(p.BldgNb) {
		result.BuildingNumber = string(*p.BldgNb)
	}
	if !isEmpty(p.BldgNm) {
		result.BuildingName = string(*p.BldgNm)
	}
	if !isEmpty(p.Flr) {
		result.Floor = string(*p.Flr)
	}
	if !isEmpty(p.Room) {
		result.RoomNumber = string(*p.Room)
	}
	if !isEmpty(p.PstCd) {
		result.PostalCode = string(*p.PstCd)
	}
	if !isEmpty(p.TwnNm) {
		result.TownName = string(*p.TwnNm)
	}
	if !isEmpty(p.CtrySubDvsn) {
		result.Subdivision = string(*p.CtrySubDvsn)
	}
	if !isEmpty(p.Ctry) {
		result.Country = string(*p.Ctry)
	}
	return result
}
func CancellationStatusReason41From(p Reason) (camt029.CancellationStatusReason41, *model.ValidateError) {
	var result camt029.CancellationStatusReason41
	if p.Originator != "" {
		Nm := camt029.Max140Text(p.Originator)
		Orgtr := camt029.PartyIdentification1352{
			Nm: &Nm,
		}
		result.Orgtr = &Orgtr
	}
	if p.Reason != "" {
		err := camt029.ExternalPaymentCancellationRejection1Code(p.Reason).Validate()
		if err != nil {
			return camt029.CancellationStatusReason41{}, &model.ValidateError{
				ParamName: "Reason",
				Message:   err.Error(),
			}
		}
		Cd := camt029.ExternalPaymentCancellationRejection1Code(p.Reason)
		result.Rsn = camt029.CancellationStatusReason3Choice{
			Cd: &Cd,
		}
	}
	if p.AdditionalInfo != "" {
		var AddtlInf []*camt029.Max105Text
		err := camt029.Max105Text(p.AdditionalInfo).Validate()
		if err != nil {
			return camt029.CancellationStatusReason41{}, &model.ValidateError{
				ParamName: "AdditionalInfo",
				Message:   err.Error(),
			}
		}
		info := camt029.Max105Text(p.AdditionalInfo)
		AddtlInf = append(AddtlInf, &info)
		result.AddtlInf = AddtlInf
	}
	return result, nil
}
func CancellationStatusReason41To(p camt029.CancellationStatusReason41) Reason {
	var result Reason
	if !isEmpty(p.Orgtr) {
		if !isEmpty(p.Orgtr.Nm) {
			result.Originator = string(*p.Orgtr.Nm)
		}
	}
	if !isEmpty(p.Rsn) {
		if !isEmpty(p.Rsn.Cd) {
			result.Reason = string(*p.Rsn.Cd)
		}
	}
	if !isEmpty(p.AddtlInf) {
		for _, info := range p.AddtlInf {
			result.AdditionalInfo += string(*info)
		}
	}
	return result
}
func isEmpty[T any](s T) bool {
	var zero T // Declare a zero value of type T
	return reflect.DeepEqual(s, zero)
}

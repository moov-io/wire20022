...
func TestEndpointGapReport_Scenario1_Step1_camt_CreateXML(t *testing.T) {
	...
	xnlFileName := filepath.Join("generated", "EndpointGapReport_Scenario1_Step1_camt.xml")
...
func TestEndpointGapReport_Scenario1_Step1_camt_OMAD_CreateXML(t *testing.T) {
	...
	message.model.ReportId = InputMessageAccountabilityData
	message.model.ReportCreateDateTime = time.Now()
	message.model.AccountOtherId = "ISOTEST1"
	...
	os.Mkdir("generated", 0755)
	// jsonFileName := filepath.Join("generated", "PaymentReturn_Scenario1_Step1.json")
	xnlFileName := filepath.Join("generated", "EndpointGapReport_Scenario1_Step1_camt_OMAD.xml")
	// err = os.WriteFile(jsonFileName, jsonData, 0644)
	// require.NoError(t, err)
	err = os.WriteFile(xnlFileName, xmlData, 0644)
	require.NoError(t, err)
